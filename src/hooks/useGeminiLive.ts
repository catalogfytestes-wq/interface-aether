// Gemini Live API WebSocket Client Hook - VERSÃO COMPLETA (SEM CORTES)
    ...
    const attemptOnce = async (modelToTry: string) => {
      ...
      ws.onopen = () => {
        ...
        const buildSetupMessage = (): GeminiSetupMessage => {
          ...
          // CORREÇÃO: O campo 'model' é necessário para a URL genérica
        ...
        // Pequeno delay para garantir estabilidade antes de enviar setup
        setTimeout(() => {
          try {
...
// CERTIFIQUE-SE DE COPIAR ATÉ AQUI (FECHAMENTO DA FUNÇÃO)
